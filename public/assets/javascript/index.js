$(document).ready(function() {
    // article-container div is where all the dynamic content will go 
    var articleContainer = $(".article-container");

    // Add event listeners to "save article" and "scrape article" buttons
...
function handleArticleSave() {
    ...
    // When we rendered the article initially, we attatched a javascript 
    // object contianing the headline id to the element using the .data method. 
    // Here we retrieve that.
    var articleToSave = $(this).parents(".panel").data();
    articleToSave.saved = true;
    // Using a patch method to be semantic since this is an update to an existing 
    ...
    .then(function(data) {
        // If successful, mongoose will send back an object containing a key of 
        // "ok" with the value of 1 (which casts to 'true')
        if (data.ok) {
...
function handleArticleScrape() {
    $.get("/api/fetch")
        .then(function(data) {
            // If we are able to successfully scrape Rolling Stone and compare the articles
            // to those already in our collection, re-render the articles on the page and
